...
if st.session_state["authenticated"] == True:
    ...
    uploaded_file2 = st.file_uploader(
        "Choose a Mero 2 EAP file with a month prior to the one chosen above",
        type="xlsx",
    )
    ...
    if uploaded_file is not None:
        ...
        if uploaded_file2 is None:
            option = st.sidebar.selectbox(
                "Filter table", (" ", "CORRECT SUM", "INCORRECT SUM")
            )
            ...
            if option == " ":
                st.dataframe(
                    df.iloc[:, [0, 3, 1, 2]].style.applymap(
                        eap_roadmap.color_background, subset=["STATUS"]
                    )
                )
            ...
            if check:
                ...
                if st.button("Download"):
                    ws = eap_compare.write_to_excel(
                        uploaded_file, uploaded_file2, finalDate
                    )
                    my_yellow = openpyxl.styles.colors.Color(rgb="FFFF00")
                    my_fill = openpyxl.styles.fills.PatternFill(
                        patternType="solid", fgColor=my_yellow
                    )
                    ...
                    for row in ws.rows:
                        for cell in row:
                            ...
                            )  # Copy protection formatting
                            new_cell.alignment = (
                                cell.alignment.copy()
                            )  # Copy alignment formatting
                    ...
                    st.markdown(
                        f'<a href="data:application/octet-stream;base64,{excel_file_b64}" download={fileName}>Download manipulated file</a>',
